...
export const main = async (prompt, numAsks, apiKey = process.env.API_KEY || minimist(process.argv.slice(2)).apiKey, model = "gpt-3.5-turbo-16k") => {
    ...
    const systemMessage = "You are a helpful assistant. Your task is to generate a meaningful response based on the user's prompt and provide the best possible answer.";
    const assistantMessage = "Let's work this out in a step by step way to be sure we have the right answer.";
    const researcherRole = `You are a researcher tasked with investigating the ${NUM_ASKS} response options provided. List the flaws and faulty logic of each answer option. Let's work this out in a step by step way to be sure we have all the errors: `;

    const resolverRole = `You are a resolver tasked with 
    1) finding which of the ${NUM_ASKS} answer options the researcher thought was best 
    ...
    const resolverPrompt = resolverRole;
    
    const resolverResponse = await openai.createChatCompletion({
        model: model,
        messages: [
            { role: "system", content: systemMessage },
            { role: "user", content: `${resolverPrompt}\n\nBest Answer according to Researcher: ${researcherResponse.data.choices[0].message.content}` }
        ],
        max_tokens: 5000,
        n: 1,
        stop: null,
